...
if(gameState=PLAY){
  ...
  if (keyDown("UP_ARROW")) {
    gun.changeImage("Up",gunRight)
  }
  if (keyDown("DOWN_ARROW")) {
  gun.changeImage("Down",gunDown);
  }
  
  if (keyDown("LEFT_ARROW")) {
   gun.changeImage("Left",gunLeft)
  }
  if (keyDown("RIGHT_ARROW")){
  gun.changeImage("Right",gunRight);
  }
  
  if (keyWentDown("UP_ARROW")) {
    Bullet=createSprite(gun.x,gun.y-8,5,5);
    Bullet.addImage("BulletU",BU)
   Bullet.scale=0.2
   Bullet.velocityY=-4;
   Bullet.lifetime=400;
  }
  if (keyWentDown("DOWN_ARROW")) {
    Bullet=createSprite(gun.x,gun.y-8,5,5);
  // Bullet.changeImage("BulletD",BD)
  Bullet.addImage("BulletD",BD)
  Bullet.scale=0.2
  Bullet.velocityY=4;
  Bullet.lifetime=400;
  }
  ...
  if (keyWentDown("LEFT_ARROW")) {
    Bullet=createSprite(gun.x,gun.y-8,5,5);
  // Bullet.changeImage("BulletL",BL)
  Bullet.addImage("BulletL",BL)
  Bullet.scale=0.2
   Bullet.velocityX=-4;
   Bullet.lifetime=400;
  }
  if (keyWentDown("RIGHT_ARROW")){
    Bullet=createSprite(gun.x,gun.y-8,5,5);
  // Bullet.changeImage("BulletR",BR)
  Bullet.addImage("BulletR",BR)
  Bullet.scale=0.2
  Bullet.velocityX=4;
  Bullet.lifetime=400;
  }
  ...
  // console.log(score)
                            ...
                            if (AlienGroup.get(i).isTouching(Bullet)) {
...
function SpawnAliens(){
  ...
  if(frameCount % 60===0){
     var al12=createSprite(900,Math.round(random(50,850)),50,5)
   al12.depth=al12.depth+2;
     al12.scale=0.6;
    // al12.velocityX=-5;
    al12.velocityX = -(6 + score/100);
...
function SpawnAliens1(){
  //Call This In mousePresssedOver(start1)
  if(frameCount % 90===0){
    var al34=createSprite(-1,Math.round(random(50,850)),50,5)
   al34.depth=al34.depth+2;
     al34.scale=0.8;
    // al34.velocityX=5;
    al34.velocityX = (6 + score/100);
    var rand = Math.round(random(1,2));
    switch(rand) {
